...
void solve() {
  ...
  sort(all(P));
  sort(P.begin() + 1, P.end(), [&](pair<pll, int> a, pair<pll, int> b) {
    if (ccw(a.fi - P[0].fi, b.fi - P[0].fi) == 0) return dist(a.fi, P[0].fi) < dist(b.fi, P[0].fi);
    return ccw(a.fi - P[0].fi, b.fi - P[0].fi) > 0;
  });
  int tmp = N - 1;
  while (tmp > 0 && ccw(P[tmp - 1].fi - P[0].fi, P[N - 1].fi - P[0].fi) == 0) {
    --tmp;
  };
  reverse(P.begin() + tmp, P.end());
  rep(i, N) {
    cout << P[i].se << ' ';
  }
  cout << "\n";
